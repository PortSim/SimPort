...
fun <BuilderT : RegularNodeBuilder<NodeT, *, *>, NodeT : NodeGroup> BuilderT.track(
    ...
    scenarioScope
        .asImpl()
        .metrics
        .add(metricFactory.create(this.node) ?: error("Metric $metricFactory does not support node ${this.node}"))
    return this
}

context(metricsScope: MetricsBuilderScope)
inline fun <reified NodeT> trackAll(metricFactory: MetricFactory<NodeT>) {
    trackNotNull({ (it as? NodeT)?.let(metricFactory::create) })
}

context(metricsScope: MetricsBuilderScope)
fun trackGlobal(metricFactory: GlobalMetricFactory) {
    metricsScope.asImpl().metrics.add(metricFactory.create(metricsScope.asImpl().scenario))
}
